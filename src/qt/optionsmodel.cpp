...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeSancheziumAmount"))
        settings.setValue("nAnonymizeSancheziumAmount", 1000);
    nAnonymizeSancheziumAmount = settings.value("nAnonymizeSancheziumAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeSancheziumAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeSancheziumAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeSancheziumAmount:
            return QVariant(nAnonymizeSancheziumAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeSancheziumAmount:
            nAnonymizeSancheziumAmount = value.toInt();
            settings.setValue("nAnonymizeSancheziumAmount", nAnonymizeSancheziumAmount);
            emit AnonymizeSancheziumAmountChanged(nAnonymizeSancheziumAmount);
            break;
        default:
            break;
        }
