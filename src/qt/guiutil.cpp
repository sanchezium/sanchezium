...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("Sanchezium"))
        return false;
...
{
    // Convert Sanchezium:// to Sanchezium:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Sanchezium://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "Sanchezium:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Sanchezium.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Sanchezium.lnk
...
{
    return GetAutostartDir() / "Sanchezium.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a Sanchezium.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Sanchezium\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Sanchezium app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Sanchezium app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Sanchezium-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Sanchezium-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("Sanchezium-Qt"));
