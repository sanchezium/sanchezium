...
void Shutdown()
{
    ...
    RenameThread("Sanchezium-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Sanchezium.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Sancheziumd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeSancheziumamount=<n> " + _("Keep N Sanchezium anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Sanchezium is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Sanchezium is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Sanchezium version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Sanchezium server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Sanchezium") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Sanchezium to complete") << "\n";
    ...
    nAnonymizeSancheziumAmount = GetArg("-anonymizeSancheziumamount", 0);
    if(nAnonymizeSancheziumAmount > 999999) nAnonymizeSancheziumAmount = 999999;
    if(nAnonymizeSancheziumAmount < 2) nAnonymizeSancheziumAmount = 2;
    ...
    LogPrintf("Anonymize Sanchezium Amount %d\n", nAnonymizeSancheziumAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1SNCZ+1000 == (.1SNCZ+100)*10
       10SNCZ+10000 == (1SNCZ+1000)*10
    */
