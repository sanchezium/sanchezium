...
int nAnonymizeSancheziumAmount = 1000;
...
#else
    const char* pszModule = "Sanchezium";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Sanchezium
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Sanchezium
    // Mac: ~/Library/Application Support/Sanchezium
    // Unix: ~/.Sanchezium
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Sanchezium";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Sanchezium.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Sancheziumd.pid"));
